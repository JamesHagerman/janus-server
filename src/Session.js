...
function Session(server, socket) {
  ...
  socket.on('close', function() {
    ...
    if (self.currentRoom) {
      self.currentRoom.emit('user_disconnected', {
        userId: self.id
      });
    }
...
Session.prototype.parseMessage = function(data) {
  ...
  payload.data._roomEmit = function(method, data) {
    self.currentRoom.emit(method, data);
  };
...
// Should really be consistant with our code structure. Put exports at BOTTOM!
